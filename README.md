...
**Mode A: Cloud SQL Connector with IAM**
...
- If `DATABASE_URL` is set, it uses direct mode (Mode B or Mode C).
- If `DATABASE_URL` is not set, it uses IAM mode (Mode A).

#### Using Prisma CLI tools

Prisma CLI tools require `DATABASE_URL` to be set, even when the API connects
via IAM. Use Mode C (proxy + `DATABASE_URL`) or a direct URL from Mode B.
