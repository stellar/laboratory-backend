...
export async function seedTestData(prisma: PrismaClient): Promise<void> {
  ...
  const testContractData: TestContractData[] = [
    ...
    // Records with NULL live_until_ledger_sequence (TTL) to test cursor pagination
    // when sort_by=ttl and the sort column is NULL (common for temporary entries)
    {
      key_hash:
      ...
      // live_until_ledger_sequence intentionally omitted → NULL in DB
    },
    {
      key_hash:
